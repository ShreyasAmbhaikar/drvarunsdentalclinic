...
export function Services() {
  return (
    <section id="services" className="bg-primary-container px-6 py-14 md:px-14 md:py-16">
      ...
      <div className="mx-auto grid max-w-[1140px] grid-cols-1 gap-6 md:grid-cols-3">
        {siteConfig.services.map((service, index) => {
          ...
          return (
            <Link
              key={service.title}
              href={service.href}
              className="group flex h-full min-h-[208px] flex-col rounded-[16px] bg-white p-6 shadow-sm transition-transform duration-200 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(108,60,17,0.12)]"
            >
              <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-primary-container text-white">
                <Icon className="h-8 w-8" aria-hidden="true" />
              </div>
              <h3 className="mb-3 font-card-title text-[19px] font-bold text-text-dark">
                {service.title}
              </h3>
              <p className="mb-6 font-body-main text-[14px] leading-6 text-text-muted">
                {service.description}
              </p>
